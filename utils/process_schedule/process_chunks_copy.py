...
# from utils.prompts.variant_extraction_prompts import system_prompt_product_entries_v2n_3
from utils.prompts.variant_extraction_prompts import system_prompt_product_entries_v2n_2point5
...
def call_llm_for_one_chunk(
    ...
    # system_prompt = system_prompt_product_entries_v2n_3
    system_prompt = system_prompt_product_entries_v2n_2point5
    ...
    if start_row==20 or start_row==40 or start_row==60: 
    ...
    if is_last_chunk:
        blocks_to_consider = content["product_blocks"]
    elif not is_last_chunk:
        blocks_to_consider = content["product_blocks"][:-1]
    ...
    if final_product_entries:
        if not last_extracted_product_block_in_previous_chunk: # very defensive
            logging.info(f"No last_extracted_product_block_in_previous_chunk found on this page")
            logging.info(f"Processed rows {start_row} to {end_row}")
            return section_context_from_last_extracted_product_block_in_previous_chunk, None, tokens_used[0], tokens_used[1]
        ...
        logging.info(f"No product entries EXTRACTED from product blocks on this page")
        logging.info(f"Processed rows {start_row} to {end_row}")
        return section_context_from_last_extracted_product_block_in_previous_chunk, last_extracted_product_block_in_previous_chunk, tokens_used[0], tokens_used[1]
...
if __name__ == "__main__":
    ...
    output_folder = "/home/student2/Documents/GitHub/1st_AI_Project_Kothari/boq_extraction_2/pipeline_copy_2/outputs/Tolani_projects_IOCL_EIL_Barauni_without_rates/General_Civil"
    schedule_path = os.path.join(output_folder, "schedule_only.xlsx")
